...
class SRNNConv(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        x_recon = x_recon.view(batch_size, length, 1, h, w)
        ...
        recon_loss = self.reconstruction_loss(x_mask[:,:self.lag], x_recon[:,:self.lag], self.decoder_dist)/self.lag + \
        (self.reconstruction_loss(x_mask[:,self.lag:], x_recon[:,self.lag:], self.decoder_dist))/(length-self.lag)
